...
export default function SinglePost() {
  ...
  useEffect(()=>{  
    const getPost=async()=>{ 
      const res=await axios.get("http://localhost:5000/api/posts/"+path)//getPost is an asynchronous function that sends an HTTP GET request to the specified URL
  ...
  const handleUpdate=async()=>{
    try{
      await axios.put("http://localhost:5000/api/posts/"+path,{username: user.username,title: title,desc:desc});
  ...
  return (
    <div className="singlePost">
      <div className="singlePostWrapper">
        <img src="https://www.shutterstock.com/image-photo/bloggingblog-concepts-ideas-white-worktable-260nw-1029506242.jpg" alt="" className="singlePostImg" />
        {
          updateMode?<input type="text" value={title} className="singlePostTitle"
          ...
          /> :(
            ...
            <i className="singlePostIcon fa-regular fa-pen-to-square" onClick={()=>setUpdateMode(true)}></i>{/*on clicking this edit option the updateMode is set to true */}
            <i className="singlePostIcon fa-solid fa-trash-can" onClick={handleDelete}></i></div>
            )}
        </h1>
        )}
        <div className="singlePostInfo">
            ...
            <span className="singlePostDate">{new Date(post.createdAt).toDateString()}</span> {/*tells when the blog was created*/}
        </div>
        {updateMode?<textarea className="singlePostDescInput" value={desc} onChange={(e)=>setDesc(e.target.value)}/>:
        (
          <p className="singlePostDesc">{/*updating the posts description*/}
            {desc}
          </p> 
        )}
